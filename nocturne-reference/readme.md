...
Note: this is a **point-in-time reference pull** from the "Nocturne" project in Claude Design (kept here for local browsing/reading, not synced automatically). The actual site (`../index.html` etc.) uses its own `../styles.css` with a bespoke deep-forest-green override layered on these same tokens — see `../CONTEXT.md`.
